...
public function uploadImage() {
	...
	public function getTimetableList() {
		...
		elseif (isset($_GET['id']) && !empty($_GET['id']) && is_numeric($_GET['id']) && isset($_GET['category_id']) && !empty($_GET['category_id']) && is_numeric($_GET['category_id'])) {
			...
			echo '<div class="alert alert-block alert-danger" style="text-align:center" >Takvim Bulunamadı </div>';
			exit();
		}
	...
	public function newTimetable($image_list) {	
		...
		$pdo = Database::connect();
		$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
		$sql = "INSERT INTO timetables (name, image, date, day, color, description ,postturu_id,marka_id,category_id) values(?, ?, ?, ?, ?, ?, ? , ?,?)";
		$q = $pdo->prepare($sql);
		$q->execute(array($_POST['name'], $image_list, $_POST['date'], $_POST['day'],$_POST['color'], $_POST['description'],$_POST['postturu_id'],$_POST['marka_id'], $_POST['category_id'] ));
