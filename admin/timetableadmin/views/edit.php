...
if (isset($_POST) && $_POST) {
	...
	@@$categori_idnew = filter_var($_POST['category_id'], FILTER_SANITIZE_NUMBER_INT);
	$day = $timetable['day'];
	$color = $timetable['color'];
	$description = $timetable['description'];
        $postturu_id = filter_var($_POST['postturu_id'], FILTER_SANITIZE_NUMBER_INT);
...
<form id="myForm" class="form-horizontal" action="edit.php<?php echo $id ? ('?id=' . $id) : 'dd'; ?>&categoryedit_id=<?php echo $idgelencategoryedit_id; ?>" method="post" enctype="multipart/form-data">
<div class="form-group">
	<input type="hidden" name="marka_id" class="form-control" value="<?php echo $idgelenmarkanew; ?>" required />
...
<input type="file" name="image[]" id="fileInput"   onchange="checkTotalFileSize()" class="filestyle" data-placeholder="<?php echo $placeholder; ?>" multiple accept=".jpg, .jpeg, .png, .mp4">		</div>
	</div>
	...
	<div class="form-group date-picker-wrap">
		...
		<div class="col-sm-8">
			<div class="input-group date">
				<input  type="text" name="date" id="custom-date" class="form-control" placeholder="Tarih Seçin"  value="<?php echo $date; ?>">
...
<form id="myForm"  class="form-horizontal"  action="edit.php?newid=<?php echo $idgelenmarkanew; ?>&category_id=<?php echo $idgelencategory_id; ?>" method="post" enctype="multipart/form-data">
	<div class="form-group">
	<input type="hidden" name="marka_id" class="form-control" value="<?php echo $idgelenmarkanew; ?>" required />
	<input type="hidden" name="category_id" class="form-control" value="<?php echo $idgelencategory_id; ?>" required />
	...
	<div class="form-group date-picker-wrap">
		...
		<div class="col-sm-8">
			<div class="input-group date">
				<input required type="text" name="date"id="custom-date" class="form-control" placeholder="Tarih Seçin" value="<?php echo $date; ?>">
				<span class="input-group-addon">
					<span class="glyphicon glyphicon-calendar"></span>
				</span>
			</div>
...
<script>
    // PHP'den gelen minDate ve maxDate değerlerini JavaScript'e aktar
    var minDateStr = '<?php echo $minDate; ?>';
    var maxDateStr = '<?php echo $maxDate; ?>';

    // String formatındaki tarihleri JavaScript Date objesine dönüştür
    ...
    // Input alanına tarih seçim widget'ını bağla
    flatpickr(dateInput, {
        enable: [
            function (date) {
                // Belirtilen aralıkta olan tarihleri etkinleştir
                return date >= minDate && date <= maxDate;
            }
        ],
        dateFormat: 'd-m-Y', // Tarih formatı (gün-ay-yıl)
        locale: {
            firstDayOfWeek: 1, // Haftanın ilk günü (1: Pazartesi, 7: Pazar)
        }
    });
</script>
<script>
$(document).ready(function() {
    // Sayfa yüklendiğinde seçili olan renk ve id değerlerini al
    var selectedColor = $('#colorSelect').val();
    var selectedId = $('#colorSelect option:selected').data('id');

    // AJAX isteği ile tur_desc değerini çekiyoruz
    $.ajax({
        type: 'POST',
        url: 'get_tur_desc.php',
        data: { color: selectedId },
        success: function(response) {
            // Gelen tur_desc değerini turDescContainer içine HTML olarak yerleştiriyoruz
            $('#turDescContainer').html(response);
        }
    });

    // Select elementi değeri değiştiğinde tur_desc değerini güncelle
...
$(document).ready(function() {
    function updateTurDesc(selectedId) {
        $.ajax({
            ...
            success: function(response) {
                ...
                // Animasyonlu gösterim için class ekleyerek opacity'yi değiştiriyoruz
    ...
    // İlk durumu yüklemek için AJAX isteği ve animasyonlu gösterim
...
function checkTotalFileSize() {
    ...
    if (fileInput.files.length > 0) {
        var totalFileSize = 0;
        var maxSize = 20 * 1024 * 1024; // 20 MB (byte cinsinden)
        ...
        if (totalFileSize > maxSize) {
            ...
            fileInput.value = ''; // Dosya seçimini iptal etmek
        }
    }
}
</script>
