...
var wordMonth = new Array("Ocak", "Şubat", "Mart", "Nisan", "Mayıs", "Haziran", "Temmuz", "Ağustos", "Eylül", "Ekim", "Kasım", "Aralık");

var wordDay_mon = "Pazartesi";
var wordDay_tue = "Salı";
var wordDay_wed = "Çarşamba";
var wordDay_thu = "Perşembe";
var wordDay_fri = "Cuma";
var wordDay_sat = "Cumartesi";
...
function createTimetable(el, btn, weekNum, monthNum, yearNum) {
	...
	var date_start = (typeof el.attr('data-start') != "undefined") ? el.attr('data-start') : 'monday';
					...
					timetableString += '<div class="timetable-column-content">';					
						...
						for (var t = 0; t < timetables.length; t++) {
							if (timetables[t].start_time && timetables[t].end_time) {
								...
								timetableString += 	'<div class="timetable-item bunabirbakweekly ">'
...
for (var t = 0; t < timetables.length; t++) {
	...
	if (timetables[t].image) {
		var imageList = timetables[t].image.split(', ');
	
		// Virgülleri kaldırmak için imageList elemanlarını birleştir
		var concatenatedImages = imageList.map(function(imagePath) {
...
// Döngü içinde dosya türüne göre işlem yapma
for (var gundondur = 0; gundondur < concatenatedImages.length; gundondur++) {
    var imagePath = concatenatedImages[gundondur];
    var fileExt = imagePath.split('.').pop().toLowerCase();

    if (fileTypes[fileExt] === 'image') {
        // Eğer dosya bir fotoğraf ise
        images.push(`
            <div class="timetable-image 2 col-4" style="float:left;">
                <a href="admin/timetable/images/${imagePath}" target="_blank" download>
                    <img style="width: 100%; height:auto; min-height:256px; max-height:256px; object-fit: contain;" src="admin/timetable/images/${imagePath}" alt="2 li foto">
                    <div style="text-align: center;">
                        <span class="download-icon" onclick="downloadFile('${imagePath}', 'image')"><i style="font-size:25px;" class="fa fa-download"></i></span>
                    </div>
                </a>
            </div>
        `);
    } else if (fileTypes[fileExt] === 'video') {
        // Eğer dosya bir video ise
        videos.push(`
            <div class="timetable-image 2 col-4" style="float:left;">
                <a href="admin/timetable/images/${imagePath}" target="_blank" >
                    <video style="width: 100%; height: auto; object-fit: contain; min-height:256px; max-height:256px;  margin-bottom: 15px; margin-top: 15px;" controls>
                        <source src="admin/timetable/images/${imagePath}" type="video/mp4">
...
function downloadFile(filePath, fileType) {
    ...
    link.href = 'admin/timetable/images/' + filePath;
    link.target = '_blank';
    link.download = filePath;

    if (fileType === 'video') {
        link.download = filePath + '.mp4';
    }
    ...
    link.click();
						...
						timetableString += 	'<div class="timetable-item">'
												+ '<span class="timetable-color" style="background-color:' + timetables[t].color + ';"></span>'
												+ '<a class="timetable-title" href="#timetable-popup-' + timetables[t].id + '" class="open-popup-link">'
													+ '<span class="timetable-name">' + timetables[t].name + '</span>'
												+ '</a>'
												+ '<div id="timetable-popup-' + timetables[t].id + '" class="timetable-popup zoom-anim-dialog mfp-hide">'
													+ '<div class="popup-header" style="background-color:' + timetables[t].color + ';">'



													+'<div class="row"><div class="col-2"><p style="font-size: larger;font-size: 17px;">' + timetables[t].postturu +' </p></div><div class="col-9"><p style="font-size: larger;padding-right: 40px;    font-size: 20px;">'+ timetables[t].name +'</p></div></div>'
													+ '</div>'
													+ '<div class="popup-body">'
													+ '<div class="row" style="border: 1px solid black; margin-bottom: 25px; border-radius: 15px; padding: 10px;  margin-left: 15px; margin-right: 15px;">'
													+ '<div class="col-3" style="display: flex; align-items: center; justify-content: center; text-align: left;">'
   													+ ' <img  class="logo-img" style="width: 120px; height: 80px; object-fit: contain;" src="admin/markalar/uploads/' + timetables[t].gelenmarkalogo + '" alt="Logo">'+ '</div>' +
													'<div class="col-6" style="text-align:center;"> <h1 class="ortalih1">'  + timetables[t].gelenmarkaadi +  '</h1></div>' + 
													'<div class="col-3" style="text-align:right;"> <h2 class="ortalih2">'  + timetables[t].date + '.' + timetables[t].month + '.' + timetables[t].year + '</h2></div>' 		
													+ '</div>'+ ' <div class="col-12" > <div class="timetable-posttur">  </div> <hr style="border: 1px solid red;"> </div>'
														+ images +  videos  + '<div style="clear:both;" > </div>'
														+ '<div  class="timetable-desc popup-header " style=" background-color:white !important; color:black  !important; margin-top: 40px; padding: 30px; font-size: 16px; ">' + timetables[t].description + '</div>'
	...
	jQuery('.tiva-timetable').each(function(index) {
		...
		jQuery.ajax({
			...
			success: function(data) {
				...
				// Create timetable
				var todayDate = new Date();
				var date_start = (typeof timetable_contain.attr('data-start') != "undefined") ? timetable_contain.attr('data-start') : 'monday';
				if (date_start == 'sunday') {
